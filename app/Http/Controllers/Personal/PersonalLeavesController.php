...
class PersonalLeavesController extends Controller
{
    ...
    public function requestL(Request $request) 
    {
        ...
        //Check leave limit
        $checkLimit = table::leavetypes()->where('id', '=', $typeid)->first();
        dd($checkLimit);
        //Check how many leaves employee already taken
        $empLeave = table::leaves()->where('reference', '=', $id)->where('typeid', '=', $typeid)->get()->count();
        if ($checkLimit->limit == $empLeave) {
            return redirect('personal/leaves/view')->with('error', 'You already avail your leaves');
        }
